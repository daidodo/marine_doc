...
{
    {
        ...
        ASSERT_FALSE(s.valid())<<ISocket::ErrMsg();   //Address family not supported by protocol
        ASSERT_EQ(-1, s.fd());
        ASSERT_TRUE(CUdpSocket::kFdType == s.fdType());
        ASSERT_STREQ("CUdpSocket", s.fdTypeName());
    }{
...
{
    {
        ...
        ASSERT_EQ("127.0.0.1:8000", addr.toString());
        {
            ...
            ASSERT_TRUE(s.bindAddr(addr))<<ISocket::ErrMsg();
            ASSERT_TRUE(s.valid());
            ASSERT_NE(-1, s.fd());
...
{
    {
        ...
        ASSERT_EQ("127.0.0.1:8000", addr.toString());
        {
            ...
            ASSERT_TRUE(s.connectAddr(addr))<<ISocket::ErrMsg();
            ASSERT_TRUE(s.valid());
            ASSERT_NE(-1, s.fd());
            ...
            //re-connect
            {
                ...
                ASSERT_TRUE(s.connectAddr(addr2))<<ISocket::ErrMsg();
                ASSERT_TRUE(s.valid());
                ASSERT_NE(-1, s.fd());
                ...
                ASSERT_FALSE(s.connectAddr(addr2))<<ISocket::ErrMsg();  //Address family not supported by protocol
                ASSERT_TRUE(s.valid());
                ASSERT_NE(-1, s.fd());
            }
        }
        ...
        ASSERT_EQ("[::ffff:127.0.0.1]:8000", addr.toString());
        {
            ...
            ASSERT_FALSE(s.connectAddr(addr))<<ISocket::ErrMsg();   //Address family not supported by protocol
            EXPECT_TRUE(s.valid());
            EXPECT_NE(-1, s.fd());
        }{
            CUdpSocket s(AF_INET);
